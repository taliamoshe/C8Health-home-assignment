...
const { validateFull, validatePartial } = require('../services/KnowledgeValidation');

const router = express.Router();
const service = new KnowledgeService();
const controller = new KnowledgeController(service);

router.post('/v1/', validateFull, controller.create);  // Create new knowledge item
...
router.patch('/v1/:id', validatePartial, controller.update);  // Update knowledge item by ID (PATCH) - partial validation
